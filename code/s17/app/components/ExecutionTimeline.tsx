          ...
          return (
            <div key={`${log.nodeId}-${i}`} className="flex items-center gap-2">
              ...
              <div className="flex-1 h-5 relative bg-zinc-800/30 rounded">
                {/* 重试区间标记 */}
                {log.retryHistory.map((retry, ri) => {
                  const retryLeft =
                    ((retry.startedAt - startTime) / totalDuration) * 100;
                  const retryWidth = Math.max(
                    ((retry.finishedAt - retry.startedAt) / totalDuration) * 100,
                    0.5,
                  );
                  return (
                    <div
                      key={ri}
                      className="absolute h-full bg-orange-500/20 rounded"
                      style={{
                        left: `${retryLeft}%`,
                        width: `${retryWidth}%`,
                      }}
                      title={`重试 #${retry.attempt}: ${retry.error.message}`}
                    />
                  );
                })}
                ...
                <div
                  ...
                  title={`${log.nodeType} — ${log.duration}ms${log.retryCount > 0 ? ` (${log.retryCount} 次重试)` : ""}`}
                />

                {/* 重试次数标签 */}
                {log.retryCount > 0 && (
                  <div
                    className="absolute top-0 text-[8px] text-orange-300 bg-orange-900/50 px-0.5 rounded"
            ...
            <div>
              <div className="text-red-400 mb-0.5">
                ...
                <span className="ml-2 text-zinc-600">
                  [{selectedLog.error.classification}]
                </span>
              </div>
...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { label: string; color: string }> = {
    ...
    retrying: { label: "重试中", color: "bg-orange-700 text-orange-100" },
  };
