"use client";

/**
 * Build An Agent - s15: 执行日志
 *
 * Input 节点组件。
 * 蓝色边框标识类型。
 */

import type { WorkflowNode } from "../../engine/types";
import { useNodeStatus } from "../Canvas";

interface Props {
  node: WorkflowNode;
  selected: boolean;
  onSelect: () => void;
}

export default function InputNode({ node, selected, onSelect }: Props) {
  const status = useNodeStatus(node.id);

  return (
    <div
      onClick={onSelect}
      className={`
        w-52 rounded-lg border bg-zinc-900 cursor-pointer transition-all
        ${selected ? "border-white ring-1 ring-white/20" : "border-zinc-700 hover:border-zinc-500"}
      `}
    >
      <div className="h-1 rounded-t-lg bg-sky-500" />
      <div className="px-3 py-2">
        <div className="flex items-center gap-2 mb-1">
          {status && <StatusDot status={status} />}
          <span className="text-xs font-medium text-sky-400">Input</span>
        </div>
        <div className="text-sm text-zinc-200">
          {(node.data.label as string) || "输入"}
        </div>
      </div>
      <div className="absolute -right-1.5 top-1/2 w-3 h-3 rounded-full bg-zinc-700 border-2 border-zinc-900" />
    </div>
  );
}

function StatusDot({ status }: { status: string }) {
  const color =
    status === "running"
      ? "bg-yellow-400 animate-pulse"
      : status === "success"
        ? "bg-green-400"
        : status === "error"
          ? "bg-red-400"
          : "bg-zinc-600";
  return <div className={`w-2 h-2 rounded-full ${color}`} />;
}
