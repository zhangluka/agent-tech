...
function ToolEditor({
  ...
  const currentTool = (node.data.tool as string) || "read_file";
  const riskLevel = getRiskLevel(currentTool);
  ...
  return (
    <div>
      ...
      {/* s16: 风险等级提示 */}
      <div className={`mt-1.5 text-[10px] flex items-center gap-1 ${riskDisplay.color}`}>
        <span className={`w-1.5 h-1.5 rounded-full ${
          riskLevel === "dangerous" ? "bg-red-400" : riskLevel === "moderate" ? "bg-yellow-400" : "bg-green-400"
        }`} />
        风险等级: {riskDisplay.label}
        {riskLevel === "dangerous" && (
          <span className="text-zinc-600"> (执行需确认)</span>
        )}
      </div>
    </div>
  );
